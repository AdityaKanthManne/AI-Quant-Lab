...
class QuantitativeSignalAgent(BaseAgent):
    async def run(self, ticker: str) -> AgentResult:
        rows = await self.provider.price_history(ticker)
        signals = calculate_signals(rows)
        metrics = [
            Metric(name=name, value=value, unit="ratio", citation_ids=[])
            for name, value in signals.items()
        ]
        return AgentResult(
            summary=f"Deterministic price and volume signals calculated for {ticker}.",
            metrics=metrics,
            warnings=["Derived metrics inherit provenance from the underlying price dataset."],
        )
